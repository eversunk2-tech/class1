...
* **활동 시간: 앱 전체를 학생이 7분 안에 마칠 분량**으로 구성한다. `spec.md`에 단계별 예상 소요 시간표를 적고(합계 7분 이하), Review에서 실제로 진행해 확인한다.
* **예상하기: 탐구형 질문 1개만.** 답을 절대 미리 제시하지 않고, 학생이 직접 짧게 타이핑한다. 힌트 버튼을 둔다(힌트도 답을 알려주지 않는다).
* **실험하기**: 지도서의 핵심 실험을 3D 장면으로 보여주고 드래그로 방향을 바꿀 수 있게 한다(three.js CDN, 터치 드래그). 조건 바꾸기·측정은 **수업 목표에 꼭 필요한 만큼만**(반복 측정·조건 수를 7분 안에 들어오게 제한). 실시간 연출은 짧게 하고, 기다림이 길면 "빨리 감기(모형)"로 줄인다. 측정값은 **기록** 버튼으로 저장한다.
...
* 마지막의 '더 탐구하고 싶은 점(또는 궁금한 점)'은 **선택 입력 한 줄**로 둔다(비워도 마칠 수 있음).
...
* 이 기준은 2026-09-22부터 적용한다. 이미 만든 앱 중 1단원 전체와 2단원 탐구 1·2·6은 이전 기준 그대로 둔다(수업에서 이미 사용했거나 사용하지 않음).
...
* 학생 화면에 "지도서"라는 말을 쓰지 않는다(교사용 자료). 출처는 "교과서·실험관찰"로 적는다.
* 측정값은 학생에게 반올림을 시키지 않고 처음부터 소수 첫째 자리로 제시한다. 측정은 조건별로 한 번씩만(반복 측정은 수업 목표에 꼭 필요할 때만).
