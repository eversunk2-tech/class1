...
* 사용자가 어떤 단계에 대해 "검토 끝나면 바로 커밋·푸시해줘"라고 미리 허락하면(09-25~26 단계 B·C·크게 보기 개편, 09-26 확대 화살표·선생님 글 링크/쪽 나누기·탐구3 옆에서 본 모습·탐구3 재수정 fix-3 — "검토 끝나면 바로 배포해줘"), **그 단계에 한해** Review에서 막히는 문제가 없거나 고친 뒤 확인 없이 커밋·push한다. 허락은 다음 단계로 넘어가지 않는다 — 새 단계는 시작할 때 다시 묻는다.
