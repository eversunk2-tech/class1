# Build(수정) 서브에이전트 지침: sci-6-1-1-4 review 1차 수정

## 목표
`public/apps/sci-6-1-1-4/review.md`의 **치명·주요·중간·경미 문제를 모두** 수정한다(참고 항목은 판단해 반영하고 보고). 수정 제안을 기본으로 하되 더 나은 방법이면 이유를 적는다.

## 수정 범위
`public/apps/sci-6-1-1-4/**`만. 공통 틀 `scripts/templates/science-sim/`과 앱의 `science-sim/` 사본은 **수정하지 않는다**(다른 앱이 공유). 틀 수정이 꼭 필요하면 앱 전용 코드로 처리하고 보고서에 "틀 반영 제안"으로 적는다.
