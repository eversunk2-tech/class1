# Build 서브에이전트 지침: sci-6-1-1-6

## 목표
승인된 `public/apps/sci-6-1-1-6/spec.md`(확정 결정 절 우선)대로 조사 도우미 앱을 만든다.

## 공통 틀
공통 틀 `scripts/templates/science-guide/`(탐구 5 작업에서 완성됨)를 복사해 쓴다. 틀은 수정하지 않는다. 필요한 기능은 앱 전용 코드로 처리하거나 보고서에 적는다.

## 반드시 먼저 읽을 것
* `CLAUDE.md` — 웹앱 규칙, **과학 차시 앱 규칙** 전체.
* `public/apps/sci-6-1-1-6/spec.md`
...
* 공통 틀 README (`scripts/templates/science-sim/README.md`, `scripts/templates/science-guide/README.md`)
...
## 수정 범위
* 수정 가능: `public/apps/sci-6-1-1-6/**`(단 `spec.md`, `*-instructions.md` 수정 금지).
...
## 완료 보고
`public/apps/sci-6-1-1-6/build-report.md`: 파일 목록, spec과 다른 점, 확인한 것/못 한 것.
