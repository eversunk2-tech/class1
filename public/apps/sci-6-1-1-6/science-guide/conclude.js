 ...
 *   '학습 마치기'를 누를 때도 저장될 글(마지막으로 제출한 글)을 한 번 더 본다(AnswerCheck.registerFinish) —
 *   block이면 마칠 수 없고, 카드는 정리하기 화면에서 보여 준다(lesson.js가 그 단계를 열어 준다).
 */
(function () {
  "use strict";
  ...
  SciSim.Conclude = {
    render: function (root, items, store, onChange, options) {
      ...
      /* ── '학습 마치기' 검사 ──
       * 저장될 글(마지막으로 제출한 글 = sent)을 한 번 더 본다. block이면 마칠 수 없다.
      ...
      if (AC && AC.registerFinish) {
        AC.registerFinish(root, function (reveal) {
          ...
          function step() {
            ...
            var text = String(st.sent == null ? st.text || "" : st.sent).trim();
            if (text.length < MIN) {
              if (reveal) reveal();
