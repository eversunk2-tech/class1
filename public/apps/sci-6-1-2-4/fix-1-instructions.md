# Build(수정) 서브에이전트 지침: sci-6-1-2-4 review 1차 수정

## 목표
`public/apps/sci-6-1-2-4/review.md`의 **치명·주요·중간·경미 문제를 모두** 수정한다(참고 항목은 판단해 반영·보고).
* 발전 질문을 이 실험 결과를 활용하는 문항으로 바꾸고, 빠진 지도서 216·217쪽 활동을 반영.

## 수정 범위
`public/apps/sci-6-1-2-4/**`만. 공통 틀 `scripts/templates/science-sim/`과 앱의 `science-sim/` 사본은 **수정 금지**(다른 에이전트가 틀을 수정 중이며, 코디네이터가 나중에 동기화). 틀 수정이 필요하면 앱 코드로 처리하고 보고서에 '틀 반영 제안'으로 적는다.
