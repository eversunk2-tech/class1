# Build(수정) 서브에이전트 지침: sci-6-1-2-1 review 1차 수정

## 목표
`public/apps/sci-6-1-2-1/review.md`의 **치명·주요·중간·경미 문제를 모두** 수정한다(참고 항목은 판단해 반영·보고).
* 주요 문제(상대 운동을 가르치는 발전 질문 2)는 지도서 192쪽 지도 원칙을 따라 다른 발전 질문으로 교체하고, 힌트 3도 같은 원칙으로 수정.

## 수정 범위
`public/apps/sci-6-1-2-1/**`만. 공통 틀 `scripts/templates/science-sim/`과 앱의 `science-sim/` 사본은 **수정 금지**(다른 에이전트가 틀을 수정 중이며, 코디네이터가 나중에 동기화). 틀 수정이 필요하면 앱 코드로 처리하고 보고서에 '틀 반영 제안'으로 적는다.
