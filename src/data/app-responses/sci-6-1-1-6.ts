...
/**
 * sci-6-1-1-6 산성화가 환경에 미치는 영향을 알아볼까? (guide · 이전 기준)
 * 출처: public/apps/sci-6-1-1-6/app.js buildDetail(), data/lesson-config.js
 * detail: kind("guide" 표시), topic(주제 라벨), intro{q1,q2,hintsOpened,revised?{q1,q2}}, research{cause,damage,solution,source}(줄 1개 고정),
 *         openedFacts, share{script,checked[],reflect}, fill, quiz.{q1,q2}, why, extension(문자열 1개), conclusion, curiosity
 */
...
export const schema: ResponseSchema = {
  ...
  notes: [
    "조사 준비의 두 답은 '생각 다 적었어요'를 누른 때(개념 카드를 보기 전)의 처음 생각이에요. 그 뒤에 고쳤으면 '고친 생각'으로 따로 보여요.",
    "중간에 주제를 바꾸면 그 전에 적은 조사하기·공유 준비하기 내용은 지워져요. 지금 보이는 조사 내용은 마지막 주제의 것이에요.",
    "빈칸 채우기·서술형·발전 질문·결론, 새롭게 알게 된 점은 '제출'한 글만 저장돼요(비어 있으면 제출하지 않은 것).",
  ],
  variants: [
    {
      id: "v1",
