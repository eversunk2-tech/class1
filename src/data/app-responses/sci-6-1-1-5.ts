...
/**
 * sci-6-1-1-5 산성 용액과 염기성 용액을 이용하는 예를 찾아라! (guide)
 * 출처: public/apps/sci-6-1-1-5/app.js buildDetail(), data/lesson-config.js (+ git 이력의 이전 버전)
 * 저장 모양이 한 번 바뀌었다(2026-09-25 간략화, 저장 키 sci6115guide:v1 → v2).
 *  - v2(간략화 후, 최상위 questionSet: 2): kind:"guide", questionSet:2, intro{answer,hintsOpened}, worksheet[{name,property,use}],
 *        propertyDiffers[{name,mine,guide}], comparedWithModel(bool), share{script,reflect}, quiz.q1·q3(choice는 이미 라벨),
 *        conclusion, curiosity('더 탐구하고 싶은 점' — 정리 질문 단계 안의 필수 한 줄)
 *  - v1(간략화 전): kind:"guide", intro{answer,experience,hintsOpened}, worksheet[{name,property,use}],
 *        propertyDiffers[{name,mine,guide}], comparedWithModel(bool), share{script,reflect},
 *        quiz.q1..q4(choice는 이미 라벨), conclusion, extension{q1,q2}, curiosity(따로 있던 5단계 '궁금한 점')
 */
...
export const schema: ResponseSchema = {
  ...
  kind: "guide",
  standard: "slim",
  // 앱의 4단계(2026-09-25 간략화 — '궁금한 점' 단계는 정리 질문 안으로 합쳤다).
  ...
  notes: [
    "정리하기의 결론(간략화 전 기록은 발전 질문도)과 발표 준비의 '새롭게 알게 된 점'은 '제출'을 누른 글만 저장돼요. 비어 있으면 제출하지 않은 거예요.",
    ...
    "2026-09-25에 질문을 줄였어요(경험 질문, 정리 문제 2개, 발전 질문 2개를 빼고 '더 탐구하고 싶은 점'은 정리 질문 단계 안으로). 그 전에 마친 기록은 '간략화 전' 버전으로 보여요.",
  ],
  variants: [
    {
      id: "v2",
      ...
      items: [
        ...
        // 화면에서는 두 번째 문제("문제 2")로 나온다. 저장 키는 간략화 전과 같은 quiz.q3(같은 질문)
        ...
        textItem("wrapup", "curiosity", "curiosity", "산성 용액과 염기성 용액에 대해 더 탐구하고 싶은 점이나 궁금한 점을 한 줄로 적어 보세요.", { label: "더 탐구하고 싶은 점" }),
      ],
    },
    {
      id: "v1",
