...
* 지금은 따로 떨어진 5번째 단계(`data-stage="curiosity"`)다. **새 기준 앱과 같은 모양**으로: 실험 앱은 **정리하기(`conclude`) 섹션 안**, 조사 앱은 **마지막 정리 단계(`wrapup`) 섹션 안** — 결론을 제출한 뒤 보이는 `finish-wrap` 안에 [더 탐구하고 싶은 점 한 줄 입력] + [🎉 학습 마치기 카드] 순서, 그 아래 `done-card`와 "↺ 처음부터 다시 하기"(`sci-6-2-1-4/index.html` 81~98행, `app.js` 107~128행과 같게).
