...
## 개정 (2026-09-26) 체험 모드도 Gemini 판정 — 사용자 결정 **최우선**
사용자: "실험앱에서 물음에 대한 학생의 응답에 제미나이api가 판단해주는 것을 지금은 로그인 했을 때만 작동하는 것으로 되어 있는데, 로그인 하지 않았을 때도 작동하도록 해줘"
* 체험 모드(비로그인 · 사이트 잠금 꺼짐)도 `check-answer`를 부른다 — 로그인 토큰 대신 **공개 키**(`Class1Record.callFunction(…, { allowAnon: true })`: 세션이 없을 때만 공개 키, 다른 사람으로 로그인된 경우 등은 그대로 실패). 판정 기록(되짚음·차단 횟수)은 이 기기에만(체험 모드는 학습 기록을 서버에 올리지 않음 — 그대로).
* 서버(`check-answer`): 토큰 종류 `user`/`anon`/그 밖(거부). 비로그인 호출에만 ① 허용 출처(CORS 목록)만 ② IP별 1분 60번·1시간 600번(학교는 한 반이 같은 IP), 인스턴스 전체 1분 300번 — 넘으면 `rate_limited`(학생은 막히지 않음) ③ Secret `CHECK_ANSWER_ALLOW_ANON=off`면 받지 않음. IP는 기록하지 않음, 로그에 `who` 추가. 보내는 데이터는 그대로(질문·답·모범 답안 — 식별 정보 없음).
* 알려진 한계: 공개 키는 누구나 볼 수 있어 사이트 밖(스크립트)에서도 부를 수 있다 — 한도는 인스턴스 메모리 기준이라 완전하지 않다. Gemini 키는 무료 등급이라 요금은 없지만 무료 한도를 다 쓰면 판정이 잠시 멈춘다(학생은 막히지 않음). 문제가 보이면 Secret으로 끈다.
