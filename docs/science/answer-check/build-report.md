...
# 개정 2 (2026-09-23) — "✔ 잘 적었어요"는 검사를 통과한 글에만

배포한 뒤 사용자가 확인한 문제: **`ㅇㅇㅇㅇㅇ` 같은 무의미한 글도 글자 수만 채우면 "✔ 잘 적었어요."가 떠서** 학생이 "답이 받아들여졌다"고 오해했다. 차단 자체는 정상 동작했다.

## 1. 고친 것

| 파일 | 변경 |
|---|---|
| `science-sim/answer-check.js`(+ guide 사본) | `passedFor(key, text, question)` 추가 — **통과한 그 글자 그대로**(`okFp` 일치)이고 로컬 무의미 규칙에도 걸리지 않을 때만 `true`. 통과 경로를 모두 `pass()` 한 곳으로 모아 **실제로 통과한 순간에만** `okFp`를 남긴다(Gemini ok · 모범 답안 낱말 지름길 · 되짚기 뒤 "그대로 제출" · 되짚기를 이미 본 질문의 통과 · 교사 확인). **"다시 써 볼게요"를 고르면 통과로 기록하지 않는다.** |
| `science-sim/predict.js`(+ guide 사본) | 글자 수 안내를 세 가지로 나눴다: ① 너무 짧음 → **문구 그대로**(`N글자 이상 적어 주세요. (지금 n글자)`) ② 길이는 채웠지만 아직 통과 전 → **중립 안내 "다 적었으면 다음 단계를 눌러요."**(+ 기존 `cfg.lengthTip`) ③ 그 글이 검사를 통과함 → **"✔ 잘 적었어요."**. 검사 직후 안내를 다시 그리고(`box.draw()`), 통과한 뒤 글을 고치면 즉시 ②로 돌아간다. **`answer-check.js`가 없는 앱은 예전 그대로**(길이만 채우면 ③). |
| `science-guide/share-prep.js` | 발표 대본 칸의 "✔ 잘 적었어요."도 중립 안내로 바꿨다(이 칸은 답 검사 대상이 아니므로 통과 여부를 알 수 없다). |
| `conclude.js`(sim·guide) | **변경 없음** — 정리하기에는 칭찬처럼 들리는 안내가 원래 없다(너무 짧을 때 문구와 제출 뒤 모범 답안 비교뿐). 브라우저로 실제 확인했다. |

`is-ok`(초록 체크 스타일) 클래스도 통과했을 때만 붙는다. **저장 키·저장 구조·`detail.qa` 모양은 그대로**이고, 기존에 저장된 `answerCheck` 값도 그대로 읽힌다(새 필드 없음).

## 2. 확인한 것 (전용 포트 8934, 가짜 세션·가짜 응답, 실제 Gemini 호출 없음)

| 시험(예상하기) | `sci-6-2-2-1` | `sci-6-1-1-1`(질문 2개) | `sci-6-2-3-2` | `sci-6-1-1-5`(조사) |
|---|---|---|---|---|
| 무의미(`ㅇㅇㅇ…`/숫자만)로 길이만 채움 | 중립 안내 | 중립 안내(두 칸 모두) | 중립 안내 | 중립 안내(두 칸 모두) |
| 그대로 '다음 단계' | 차단 카드(서버 호출 0) | 차단 카드(0) | 차단 카드(0) | 차단 카드(0) |
| 좋은 글로 고친 직후(검사 전) | 중립 안내 | 중립 안내 | 중립 안내 | 중립 안내 |
| 통과한 뒤 | **✔ 잘 적었어요** | **✔ 잘 적었어요**(두 칸) | **✔ 잘 적었어요** | **✔ 잘 적었어요**(두 칸) |

- **되짚기(rethink) 흐름**: 카드가 뜬 동안·"다시 써 볼게요"를 고른 뒤에는 **칭찬 없음**, 다시 제출해 통과한 뒤에야 "✔ 잘 적었어요". Gemini 응답 없음(시간 초과 3,553 ms) → "그대로 제출할게요"로 통과한 뒤에만 칭찬.
- **통과 뒤 글을 고치면** 곧바로 중립 안내로 돌아간다. 앞 단계로 돌아와도 통과 상태는 유지된다(새로고침·이어서 하기 포함, `okFp` 저장).
- **정리하기**: `sci-6-1-1-5`에서 무의미 답 → 차단 카드 + 모범 답안 계속 감춰짐, 화면에 칭찬 문구 0건, 좋은 답 제출 → 모범 답안 공개.
- **발표 준비 대본 칸**: "다 적었으면 다음 단계를 눌러요."(칭찬 없음).
- **회귀**: `answer-check.js`를 뺀 사본은 예전 그대로 길이만 채우면 "✔ 잘 적었어요" + 그냥 통과.
- 콘솔 오류 0건. 로컬 규칙 28/28 · 말뭉치 오차단 0건 · 함수 34/34 · 로그인 확인 15/15 그대로 통과.
- 정본 → **23개 앱 재복사, `diff -r` 전부 일치**. `app.js`·`lesson-config.js` 변경 0건.

## 3. 사용자가 할 일

배포 절차는 그대로다(§10 / 개정 1 §3). 이 변경은 **사이트 파일만** 바뀌므로 Edge Function을 다시 배포할 필요가 없다.
