# Build 공통 지침: 6학년 2학기 3단원 "전기의 이용" 시뮬레이션 앱 (2026-09-23, 사용자 승인)

앱 3개(sci-6-2-3-1 ~ 3)를 **서로 다른 Build 서브에이전트가 동시에** 만든다. 자기 앱 폴더만 다룬다.
...
* 본보기: **2학기 2단원 앱** `public/apps/sci-6-2-2-1/`~`4/`, **2학기 1단원 앱** `public/apps/sci-6-2-1-2/`~`5/`(값 패널·기록하기·시간 바·슬라이더, `review.md`·`fix-1-report.md`에서 고친 점 참고), 색·관찰형은 1학기 산과 염기 앱 `sci-6-1-1-1/`, `sci-6-1-1-2/`. 리뷰에서 반복된 문제를 처음부터 피한다: 힌트가 답을 말함, 빨리 감기·단순화("모형") 미표시, 수업 목표 활동 누락, 세로 화면에서 3D가 화면 밖, WebGL 미해제, 학생 입력 innerHTML 삽입, 색만으로 정보 전달, 다음 차시 스포일러.
...
* 저장 키: spec의 `sci623simN:v1`.

## 핵심 요구
* 관찰 결과는 spec 문구만(지도서·실험관찰 예시). 학생이 타이핑하지 않고 관찰 카드에 결과가 보이며 **기록하기**로 저장(관찰 보기 고르기가 spec에 있으면 그대로).
* 전기: 지도서 안전 수칙 안내(전지 양극을 전선으로 바로 잇지 않기 등), 전류·자기장은 화살표 등 "모형" 표시. 중학교 용어(전압·전류의 세기·저항) 금지, 교과서 표기(전기 회로·직렬연결·전자석)만. 다음 차시 스포일러 금지(spec 참고).
* 공통 틀은 2026-09-23 수정본(`docs/science/template-fix-1-report.md`)을 쓴다 — 앱 안에서 같은 문제를 다시 우회하지 말 것.
...
* 블로그로 돌아가기 링크: `../../science/?term=6-2&unit=3&lesson={lesson id}`.
