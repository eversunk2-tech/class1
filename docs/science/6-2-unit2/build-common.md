# Build 공통 지침: 6학년 2학기 2단원 "물질의 연소" 시뮬레이션 앱 (2026-09-22, 사용자 승인)

앱 4개(sci-6-2-2-1 ~ 4)를 **서로 다른 Build 서브에이전트가 동시에** 만든다. 자기 앱 폴더만 다룬다.
...
* 본보기: **2학기 1단원 앱** `public/apps/sci-6-2-1-2/`~`5/`(값 패널·기록하기·시간 바·슬라이더, `review.md`·`fix-1-report.md`에서 고친 점 참고), 색·관찰형은 1학기 산과 염기 앱 `sci-6-1-1-1/`, `sci-6-1-1-2/`. 리뷰에서 반복된 문제를 처음부터 피한다: 힌트가 답을 말함, 빨리 감기·단순화("모형") 미표시, 수업 목표 활동 누락, 세로 화면에서 3D가 화면 밖, WebGL 미해제, 학생 입력 innerHTML 삽입, 색만으로 정보 전달, 다음 차시 스포일러.
...
* 저장 키: spec의 `sci622simN:v1`.

## 핵심 요구
* 관찰 결과는 spec 문구만(지도서·실험관찰 예시). 학생이 타이핑하지 않고 관찰 카드에 결과가 보이며 **기록하기**로 저장(관찰 보기 고르기가 spec에 있으면 그대로).
* 불·연소: 안전 수칙 안내, 불꽃·연기·기체는 "모형" 표시, 불장난을 부추기는 표현 금지. 반응식·원자 모형 금지. 다음 차시 스포일러 금지(spec 참고).
...
* 블로그로 돌아가기 링크: `../../science/?term=6-2&unit=2&lesson={lesson id}`.
