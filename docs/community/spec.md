...
- **섹션 재구성**(2열 그리드 유지, `DashboardSection` 재사용):
  ...
  * **개정(2026-09-26, 사용자 결정)**: 홈의 '과학 수업 글'(과학 태그 글)을 '최근 과학 수업'에서 떼어 **'선생님 글'** 칸으로 — 관리자 화면에서 쓴 공개 글(`posts`) 전부를 **제목만** 목록으로 보이고 제목을 누르면 본문이 펼쳐진다(날짜·조회수 없음, `src/components/dashboard/teacher-posts.tsx`, 5개씩 더 보기). 왼쪽 메뉴에는 없음. 과학수업 페이지의 '과학 수업 글' 칸은 삭제. 로그인 학생이 펼치면 읽음 기록(`post_reads`)만 남긴다.
  2. **최근 자유게시판**: `CommunityPostList kind="board" limit=3 loginOnly` — **로그인한 사용자에게만**(2026-09-26 사용자 결정: "로그인을 하지 않으면 자유게시판 미리보기는 되지 않도록"). 로그인하지 않은 방문자에게는 목록을 불러오지 않고 "로그인하면 볼 수 있어요 — 최근 자유게시판 글은 로그인한 우리 반 친구들에게만 보여요" 안내.
  * **개정(2026-09-26, 사용자: "자유게시판은 로그인 하지 않으면 완전히 안 보여야해. 메뉴 선택해도 글이 보이지 않도록")**: 자유게시판 전체가 로그인 전용 — `/board/`(목록·글·글쓰기·고치기)는 `LoginGate`가 로그인 안내("로그인이 필요해요"), 홈 '자유게시판 글' 수 타일도 `loginOnly`(비로그인 "로그인 필요"), RLS `20260926000000_board_login_only.sql`(비로그인은 `kind='board'` 글·댓글·좋아요 SELECT 불가). 학습게임은 "로그인해야만 이용" 스위치대로.
