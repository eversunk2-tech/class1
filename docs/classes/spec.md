...
## 개정 2 — 학급별 '선생님 글' (2026-09-26 사용자 결정) **최우선**
사용자: "학생마다 자기 담임선생님 글만 보게 해줘(로그인 안했을 때는 로그인 하면 확인 가능하다는 표시)" · "읽음 기록은 필요 없어" · "관리자 대시보드의 '글관리-새글작성'의 구조가 너무 복잡해. 이것을 학급별 '선생님 글' 게시판 작성하는 곳으로 해서 간단하게 제목과 본문만 담임교사가 작성할 수 있게 해줘"
(이미 배포된 것 — 커밋 `f16703f`·`07e5fc4`: 홈 '선생님 글' 칸(제목만, 누르면 본문, 날짜·조회수·읽음 기록 없음), 로그인하지 않으면 "로그인하면 담임 선생님 글을 확인할 수 있어요", 과학수업 페이지의 '과학 수업 글' 칸 삭제. 지금은 블로그 글(`posts`)을 읽는다 → 아래로 바꾼다.)

### 2-1. 무엇을
- **새 표 `public.class_notices`**(학급 공지): `id uuid pk default gen_random_uuid()`, `class_id uuid not null references public.classes(id) on delete cascade`, `author_id uuid references public.profiles(id) on delete set null`, `title text not null`(앞뒤 공백 제거 1~100자), `body text not null`(1~5000자), `created_at`, `updated_at`(트리거로 갱신). **제목·본문만**(태그·주소(slug)·요약·표지·공개 여부 없음 — 쓰면 바로 보임).
- **보기(RLS SELECT)**: 학생 = **자기 학급 글만**(학생의 반은 `profiles.class_id` — 공개 열이 아니므로 `security definer` 함수 `my_student_class_id()`(auth.uid()의 `class_id`, 학생이 아니면 null)로), 담임(총괄 포함) = 자기 학급(`my_class_ids()`) 글. 비로그인 = 없음. 다른 반 글은 누구도 못 봄(총괄도 자기 반만 — 개정 1과 같은 원칙).
- **쓰기(INSERT/UPDATE/DELETE)**: 그 학급 담임만(`class_id = any(my_class_ids())`), `author_id = auth.uid()`(INSERT). 학생·비로그인 불가.
- 읽음 기록·조회수·댓글·좋아요 없음.

### 2-2. 화면
- **홈 '선생님 글'**(`src/components/dashboard/teacher-posts.tsx`): `posts` 대신 `class_notices`를 읽는다(RLS가 자기 학급 글만 돌려줌). 제목만 목록, 누르면 본문이 펼쳐짐, **날짜 없음**. 본문은 **글자 그대로**(줄바꿈 유지, 마크다운·HTML 해석 안 함 — 간단·안전). 비로그인 = "로그인하면 담임 선생님 글을 확인할 수 있어요"(지금과 같음). 학급이 없는 학생·글이 없으면 "아직 선생님 글이 없어요". 홈 통계 칸 '선생님 글'도 `class_notices` 개수(비로그인 "로그인 필요").
- **관리자 대시보드**: '글 관리'·'새 글 작성'(블로그 편집기)을 없애고 **'선생님 글'** 한 곳으로 — 내 학급 글 목록(학급이 여럿이면 학급 고르기), **새 글(제목·본문 두 칸만)**, 고치기, 지우기(확인, 위험 버튼 규칙). 학급이 없는 담임에게는 "먼저 학급을 개설해 주세요". 관리자 목록에는 쓴 날짜를 작게 보여도 된다(관리용 — 학생 화면에는 없음).
- 기존 블로그(`posts`·`/post/`·댓글·좋아요·조회수·검색·태그)는 **지우지 않고 화면에서만 뺀다**(관리자 메뉴·개요 바로가기에서 '글 관리'·'새 글 작성' 제거). 공개 글 0개(09-26 기준). 상단 검색(블로그 글 검색)·`/post/` 주소를 없앨지는 사용자에게 따로 묻는다.

### 2-3. 순서
- 새 SQL `supabase/migrations/20260927020000_class_notices.sql`(① schema·② 부엉이반·③ rls 다음 ④로 실행 — `classes`·`my_class_ids()`가 필요). 화면은 ①~④ 실행·함수 재배포 뒤 push.

### 개정 2 보충 — 비로그인에게는 총괄 선생님 글 (2026-09-26 사용자 결정) **개정 2의 "비로그인 = 없음"을 대체**
사용자: "로그인 안했을 때는 총괄 관리자가 작성한 선생님 글이 보이게 해줘"
- RLS SELECT에 **"글쓴이가 총괄이면 누구나(비로그인 포함) 읽음"**을 더한다(`security definer` 도우미 `is_super_admin_id(author_id)` — `profiles.is_super_admin`은 공개 열이 아니므로). 쓰기 규칙은 그대로(그 학급 담임만).
- 홈: 비로그인에게도 목록(총괄 글) — **안내 한 줄은 넣지 않는다**(사용자: "'로그인하면 우리 반 선생님 글을 볼 수 있어요' 이 문구는 없어도 돼"). 로그인한 학생은 지금 설계대로(RLS가 돌려주는 자기 학급 글). 홈 통계 칸도 누구에게나 RLS가 보여 주는 개수.
- 관리자 '선생님 글'에서 총괄에게 "총괄 선생님 글은 로그인하지 않은 방문자에게도 보여요." 안내.
- **확정**(사용자: "다른 반 학생들이 처음부터 로그인 하고 있을 경우 총괄의 글은 보지 않아도 돼"): "글쓴이가 총괄이면 읽음"은 **비로그인에게만** — RLS 예: `(auth.uid() is null and is_super_admin_id(author_id)) or class_id = my_student_class_id() or class_id = any(my_class_ids())`. 로그인한 다른 반 학생은 자기 담임 글만.
- **잠금 스위치(확정)**(Build 담당이 정한 것을 사용자가 확정: "담당이 정한대로 해줘"): 로그인 잠금 스위치가 켜지면 비로그인에게 **총괄 글도 보이지 않는다** — 블로그·게시판·게임과 같은 잠금 규칙(RLS 비로그인 정책에 `can_browse()`, 홈은 잠금 안내).

---

## 개정 3 — 과제·글은 쓴 선생님과 총괄만 + 과제도 학급별로 (2026-09-26 사용자 결정) **최우선**
사용자: "과제·글 수정은 쓴 선생님과 총괄만 하게 해줘"(Review M1 — 개정 1-5의 열린 것) → 이어서 "1번으로 과제도 학급별로 나눠줘"
(선택지였던 것: 1 과제 학급별로 나누기 / 2 다른 반 제출물이 있으면 지우기 거부 / 3 지우기 대신 비공개만)

### 3-1. 쓴 선생님과 총괄만(⑤ `supabase/migrations/20260927030000_content_owner_only.sql`, 커밋 `b3d7ee1`)
- 과제(`assignments.created_by`)·블로그 글(`posts.author_id`): 만들기 = 교사 본인 이름으로, 고치기·지우기 = 쓴 교사 또는 총괄. 만든 사람 열은 아무도 못 바꿈(열 권한).
- 관리자 화면: 남의 것은 수정·공개 전환·삭제 꺼짐 + "쓴 선생님과 총괄만 고칠 수 있어요."

### 3-2. 과제도 학급별로(⑥ `supabase/migrations/20260927040000_class_assignments.sql` — ⑤ 다음)
- 표: `assignments.class_id`(학급, 필수). 지금 과제는 만든 선생님의 학급으로(없으면 총괄의 부엉이반).
- 보기: **학생 = 자기 학급의 공개 과제만**(로그인 잠금 `can_browse()` 그대로), **비로그인 = 없음**(과제는 반 학생용), 교사 = 자기 학급 과제 + 자기가 만든 과제, 총괄 = 전체(3-1 "총괄은 고칠 수 있음"과 맞게 — 화면 목록은 학급 범위로).
- 만들기 = **자기 학급에만**(작성자 = 나). 고치기·지우기 = 3-1 그대로. 과제의 학급은 만든 뒤 바꾸지 않는다(열 권한에서 뺌).
- 제출 = 자기 학급의 공개 과제에만 — 제출 정책이 과제를 "볼 수 있는가"로 확인하므로 보기 규칙을 따라 자동으로 좁아진다. 그래서 **과제를 지워도 그 반 제출물만** 지워진다(다른 반 연쇄 삭제 문제 해결).
- 화면: 관리자 과제 관리 — 새 과제에 학급 고르기(하나면 자동, 없으면 "먼저 학급을 개설해 주세요"), 목록은 학습 현황의 학급 범위(내 학급 전체 / 고른 학급). 학생 화면('내 학습 활동' 과제)은 그대로(RLS가 자기 반 과제만 돌려줌).
- 실행 순서: ⑤ → ⑥ → 화면 push(⑤·⑥ 모두 사용자가 SQL Editor에서).
