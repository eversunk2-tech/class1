...
- 홈: 비로그인에게도 목록(총괄 글) + 목록 아래 한 줄 "로그인하면 우리 반 선생님 글을 볼 수 있어요." 로그인한 학생은 지금 설계대로(RLS가 돌려주는 자기 학급 글). 홈 통계 칸도 누구에게나 RLS가 보여 주는 개수.
- 관리자 '선생님 글'에서 총괄에게 "총괄 선생님 글은 로그인하지 않은 방문자에게도 보여요." 안내.
- 열린 것(사용자에게 확인): 다른 반 학생(로그인)은 총괄 글을 보지 못한다(자기 담임 글만) — 비로그인 방문자보다 덜 보는 셈. 함께 보이게 할지 물어본다.
