...
- **총괄**(`profiles.role = 'admin'` + `is_super_admin = true`)은 **모든 학생**을 탈퇴 처리할 수 있습니다.
- **담임**은 **자기 학급 학생만** 탈퇴 처리할 수 있습니다(대상의 `profiles.class_id`가 호출자의 `class_teachers`에 있을 때).
  학급이 없는 회원(예: GitHub·Google로 가입한 방문자)은 총괄만 할 수 있습니다.
- 그 밖에는 **403** "이 학생의 담임 선생님이나 총괄 관리자만 탈퇴 처리할 수 있습니다." — 이 확인은 **계정을 지우기 전에** 끝납니다.
- 탈퇴해도 `profiles` 행과 학급(`class_id`)은 남으므로, 담임은 그 학생의 기록을 "탈퇴한 학생"으로 계속 보고,
  "표시만 다시 시도"(재실행)도 같은 담임이 할 수 있습니다.
...
- 배포 순서: SQL `20260927000000_classes_schema.sql` → `docs/classes/setup-owl-class.sql`(이메일을 바꿔서) →
  `20260927010000_classes_rls.sql` → `20260927020000_class_notices.sql` → **이 함수와 `admin-create-member`·`admin-reset-password` 다시 배포** → 사이트 push.
  첫 SQL 전에 새 코드를 배포하면 "학급 기능용 DB 설정이 아직 적용되지 않았습니다"로 거부합니다(계정을 지우지 않음).
...
3. 확인할 것:
   ...
   - 그 계정으로 로그인하면 실패한다.
   - 회원 상세의 **학습활동 탭(웹앱 결과·댓글·과제·피드백)이 그대로 보인다.** ← 기록 보존 확인(그 학생의 담임 계정으로)
   - 자유게시판·학습게임에서 그 학생의 글이 **"탈퇴한 학생"** 이름으로 남아 있다.
   - (두 번째 담임이 생긴 뒤) 그 담임 계정으로 부엉이반 학생을 탈퇴 처리하면 403 문구가 나오고 계정은 그대로다
     (담임 화면의 회원 명단에는 다른 반 학생이 보이지 않으므로, 이 확인은 Review 단계에서 요청을 직접 보내 확인합니다).
...
| "학급 기능용 DB 설정이 아직 적용되지 않았습니다" | `20260927000000_classes_schema.sql`을 먼저 실행하세요. (이 상태에서는 계정을 지우지 않습니다.) |
| "이 학생의 담임 선생님이나 총괄 관리자만 탈퇴 처리할 수 있습니다" (403) | 대상 학생이 호출한 선생님의 학급이 아닙니다(학급이 없는 회원 포함). 담임 또는 총괄이 하세요. |
