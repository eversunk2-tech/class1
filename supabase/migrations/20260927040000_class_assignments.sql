...
--          (총괄도 자기 학급에만. 학급 없이(class_id 빈 값) 만들면 RLS 거부(42501) — Postgres 는 INSERT 때 RLS with check 를 not null 보다 먼저 본다.)
...
--     (학급 없이 만들려 해서 RLS 거부(42501) — "과제를 저장하지 못했습니다."). 확인 쿼리 뒤 바로 push 한다.
