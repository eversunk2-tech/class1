/*
 * science-guide/answer-check.js — 학생 답 되짚기·차단 (정본: scripts/templates/science-guide/, science-sim에서 복사 — 두 파일은 같다)
 ...
 *   - Gemini가 응답하지 못하면(오프라인·시간 초과·오류·한도 초과) **절대 막지 않는다** — rethink 수준으로 낮춰 한 번만 권하고 통과시킨다.
 ...
 *
 * ▶ 앱이 할 일 — 없다
 *   index.html에 <script src="./science-guide/answer-check.js"></script> 한 줄만 있으면,
...
(function () {
  ...
  var TIMEOUT_MS = 3500; /* 클라이언트 전체 대기 한도(7분 수업 기준) */
  ...
  function verify(o) {
    ...
    return ask(payloadFor(o, st.blockCount)).then(function (r) {
      ...
      /* rethink — 카드를 본 순간 이 질문은 다시 검사하지 않는다(평생 1회) */
      st.nudged = true;
      put(key, st);
      return showCard({
        kind: "rethink",
